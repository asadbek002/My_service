...
export default function OrderPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <main className="page"><header><Link href="/orders">← Buyurtmalar</Link><span className="brand">MY SERVICE</span></header>
    ...
    {order.status === 'READY' && can('orders.edit') && <section><h2>Mijozga topshirish</h2><form onSubmit={form(d => action('/deliver', { warrantyDays: Number(d.get('days')), warrantyTerms: d.get('terms'), allowDebt: d.get('allowDebt')==='on' }))}><label>Kafolat (kun)<input name="days" type="number" min="1" max="1095" defaultValue="90" required /></label><label>Kafolat shartlari<textarea name="terms" minLength={5} required /></label>{can('payments.deliver_with_debt')&&<label className="check"><input type="checkbox" name="allowDebt"/>Qarzdorlik bilan topshirishga ruxsat</label>}<button disabled={busy}>Qurilmani topshirish</button></form></section>}
    </div>
  </main>;
}
