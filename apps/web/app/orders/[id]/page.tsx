...
type Order = { id: string; number: string; status: string; complaint: string; total: string; quoteVersion: number; diagnosis: string | null; requiredWork: string | null; customer: { firstName: string; phone: string }; device: { brand: string; model: string }; history: { id: string; toStatus: string; comment: string; createdAt: string }[]; finalTestChecklist: string[]; parts:{id:string;part:{name:string}}[];repairActions:{id:string;description:string}[] };
export default function OrderPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <main className="page"><header><Link href="/orders">← Buyurtmalar</Link><span className="brand">MY SERVICE</span></header>
    ...
    {order.status === 'READY' && can('orders.edit') && <section><h2>Mijozga topshirish</h2><form onSubmit={form(d => action('/deliver', { warrantyDays: Number(d.get('days')), warrantyTerms: d.get('terms'), coveredOrderPartIds:d.getAll('coveredParts'), coveredRepairActionIds:d.getAll('coveredActions'), allowDebt: d.get('allowDebt')==='on' }))}><label>Kafolat (kun)<input name="days" type="number" min="1" max="1095" defaultValue="90" required /></label><label>Kafolat shartlari<textarea name="terms" minLength={5} required /></label>{order.parts.map(v=><label className="check" key={v.id}><input type="checkbox" name="coveredParts" value={v.id}/>{v.part.name} kafolati</label>)}{order.repairActions.map(v=><label className="check" key={v.id}><input type="checkbox" name="coveredActions" value={v.id}/>{v.description} ish kafolati</label>)}{can('payments.deliver_with_debt')&&<label className="check"><input type="checkbox" name="allowDebt"/>Qarzdorlik bilan topshirishga ruxsat</label>}<button disabled={busy}>Qurilmani topshirish</button></form></section>}
    </div>
  </main>;
}
